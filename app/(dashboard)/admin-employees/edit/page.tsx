...
export default function EditAdminEmployeePage() {
  ...
  const loadEmployee = async () => {
    ...
    try {
      ...
      if (!employee) {
        alert("Admin employee not found");
      ...
      console.error("Error loading admin employee:", error);
      alert("Failed to load admin employee");
      router.push("/admin-employees");
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateAdminEmployee(employeeId, updateData);
      router.push("/admin-employees");
    } catch (error) {
      console.error("Error updating admin employee:", error);
      alert("Failed to update admin employee");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6">
      <h1 className="mb-6 text-3xl font-bold text-slate-100">Edit Admin Employee</h1>
      ...
      <form onSubmit={handleSubmit} className="rounded-lg bg-dark-card border border-dark-border p-6 shadow-lg" style={{ overflow: 'visible' }}>
        <h2 className="mb-4 text-xl font-semibold text-slate-100">Admin Employee Details</h2>
